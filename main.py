...
Usage:
  main.py -d <data_path_file> -w <window_size> -s <smoothing_window_size> -o <output_log_file> [--original] [--exp] [--smoothed] [--exp_smoothed] [--aaindex] [--secondary_structure]
  main.py (-h | --help)
  main.py --version
...
def grid_search(cost_range, gamma_range, crossValidation):
    for i in xrange(cost_range[0], cost_range[1]+1):
        for j in xrange(gamma_range[0], gamma_range[1]+1):
            cost  = 2 ** i
            gamma = 2 ** j
            crossValidation.run_SVM(cost, gamma)
...
if __name__ == "__main__":
    ...
    gamma_range           = (-6, 2) # 2**this_integer
    ...
    feature_info = create_feature_info(window_size, smoothing_window_size, original_pssm, 
                                    exp_pssm, smoothed_pssm, exp_smoothed_pssm, AAindex, secondary_structure)
